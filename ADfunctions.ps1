...
function Get-InactiveUsers{
    ...
    #Output users who have not logged on in $daysinactive days
    $DaysInactive = 120
    ...
    if ($service){
    Get-ADUser -Filter {LastLogonTimeStamp -lt $time -and enabled -eq $true -and Name -notlike "*#*"} -Properties LastLogonTimeStamp |
    ...
    Get-ADUser -Filter {LastLogonTimeStamp -lt $time -and enabled -eq $true -and Name -like "*,*" -and Description -notlike "*Service*" -and Name -notlike "*#*"} -Properties LastLogonTimeStamp |
    select-object Name,@{Name=”Last Log-On Date”; Expression={[DateTime]::FromFileTime($_.lastLogonTimestamp).ToString(‘yyyy-MM-dd’)}}, @{Name = "Days Till Log On";
                     Expression = {((get-date) - [DateTime]::FromFileTime($_.lastLogonTimestamp)).days}} |Out-GridView}
}
...
function Get-PasswordExpirationUsers{
    ...
    #Show users whos passwords expire within $DaysLeftFilter days
    $DaysLeftFilter = 30
    ...
    Foreach ($ADUsers in Get-ADuser -filter {(PassWordNeverExpires -eq "false") -and (Enabled -eq "True")}){ 
       # $adusers.SamAccountName
        ...
        if (($DaysToExpire -le 30)-and ($DayofExpiration -ge (get-date))) {
